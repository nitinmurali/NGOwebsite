...
<body>
  <body> 
    ...
    $query1= "select* from personal_info";

   echo ('<div class="container" style="margin-top:-5px;">
   <h1 style="color:black;align-content:center;text-transform:uppercase;margin-bottom:35px;">Personal Info</h1>
   </div>
   ...
   ');
    ...
    if ($result1 = $mysqli->query($query1)) {
        while ($row = $result1->fetch_assoc()) {
            $field1name = $row["donor_name"];
            $field2name = $row["donor_email"];
            $field3name = $row["donor_phoneno"];
            $field4name = $row["donor_id"];
        
     
            echo '
            <tbody>
            <tr>
              <th scope="row">'.$field4name.'</th>
              <td>'.$field1name.'</td>
              <td>'.$field2name.'</td>
              <td>'.$field3name.'</td>
              </tr>
    ...
    $query2= "select* from payment_details";
    echo ('<div class="container" style="margin-top:-30px;">
   <h1 style="color:black;align-content:center;text-transform:uppercase;margin-bottom:35px;">Payment Info</h1>
   </div>
   ...
   if ($result2 = $mysqli->query($query2)) {
    while ($row = $result2->fetch_assoc()) {
        $f1name = $row["donor_firstname"];
        $f2name = $row["amount_paid"];
        $f3name = $row["donor_Address"];
        $f4name = $row["donor_city"];
        $f5name = $row["donor_state"];
        $f6name = $row["donor_date"];
        $f7name = $row["donor_paymentmethod"];
        echo '
        <tbody>
        <tr>
          <th scope="row">'.$f1name.'</th>
          <td>'.$f2name.'</td>
          <td>'.$f3name.'</td>
          <td>'.$f4name.'</td>
          <td>'.$f5name.'</td>
